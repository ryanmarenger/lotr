...
const output = sites.map(s => ({
  blueprintId: s.blueprintId,
  set: s.setNumber,
  cardNumber: s.cardNumber,
  siteNumber: s.siteNumber || null,
  title: s.title,
  block: s.siteBlock,
  gameKeywords: s.keywords || [],
  terrain: assignTerrain(s)
}));

const unassigned = output.filter(o => o.terrain === 'UNASSIGNED');
console.log(`Total: ${output.length}, Unassigned: ${unassigned.length}`);
if (unassigned.length > 0) {
  unassigned.forEach(u => console.log(`  UNASSIGNED: "${u.title}" (set ${u.set}, #${u.cardNumber})`));
}

// Summary
const counts = {};
output.forEach(o => { counts[o.terrain] = (counts[o.terrain] || 0) + 1; });
